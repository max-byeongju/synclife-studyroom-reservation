...
@Slf4j
@ActiveProfiles("test")
public class NonConflictingReservationTest {
    ...
    @Test
    @DisplayName("겹치지 않는 시간대에 대한 동시 예약 요청은 모두 성공해야 한다")
    void concurrentNonConflictingReservationsSucceed() throws InterruptedException {
        // Given
        ...
        for (int i = 0; i < 5; i++) {
            ...
            executorService.submit(() -> {
                try {
                    ...
                    LocalDateTime startAt = LocalDateTime.now().plusHours(1 + timeOffset * 3);
                    ...
                    log.info("다른 시간대 예약 성공 - 사용자: {}, 시간: {} ~ {}",
                            testUsers.get(userIndex).getName(), startAt, endAt);

                } catch (Exception e) {
                    exceptions.add(e);
                    log.error("다른 시간대 예약 실패 - 사용자: {}, 오류: {}",
